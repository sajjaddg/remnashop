event-error =
    ...
    *[HAS]
    ...
    <blockquote>
    The bot cannot function properly without an active connection!
    </blockquote>
...
event-bot =
    ...
    *[1] enabled
    ...
    <b>⚠️ Event: Inline mode disabled in BotFather!</b>

    <blockquote>
    The bot is not configured for inline mode. Some bot features may not work correctly.
...
event-blacklist =
    ...
    <b>🔅 Event: Registration attempt from blacklist!</b>
...
event-subscription =
    ...
    { $is_trial ->
    [0]
    <b>⚠️ Attention! Your subscription will expire in { unit-day }.</b>
    
    Renew it in advance to avoid losing access to the service! 
    *[1]
    <b>⚠️ Attention! Your free trial will expire in { unit-day }.</b>
    ...
    *[1] { $traffic_strategy ->
    ...
    If you're having difficulties setting up VPN — we're here to help! Contact support and we'll figure it out together.
...
event-torrent-blocker =
    ...
    The restriction will remain in effect for <b>{ $block_duration }</b>.

    If you need help configuring your connection, contact support.
...
event-referral =
    ...
    <blockquote>
    User <b>{ $name }</b> joined via your invite link! To receive the reward, make sure they purchase a subscription.
    </blockquote>
    ...
    *[OTHER] <b>{ $value } { $reward_type }</b>
    ...
    .reward-failed =
    <b>❌ Failed to issue the reward!</b>
    
    <blockquote>
    User <b>{ $name }</b> made a payment, but we couldn't credit your reward because <b>you don't have a purchased subscription</b> to add { $value } { $reward_type ->
    [POINTS] { $value -> 
        [one] point
        *[other] points 
        }
...
event-payment =
    ...
    Manual verification required — the user's subscription may still be active.

    .referral-failed =
    <b>⚠️ Failed to credit referral reward</b>
    ...
    The purchase was completed successfully, but an error occurred while crediting the referral reward. Manual verification required.
    ...
    Payment received, but the subscription could not be issued. The transaction has been marked as FAILED. Manual verification required.

event-remnashop-welcome =
    <b>💎 Remnashop v{ $version }</b>

    This project is created and maintained by a single <strike>developer</strike> electrician. Since the bot is completely FREE and open source, it exists only thanks to your support.

    ⭐ <i>Star us on <a href="{ $repository }">GitHub</a> and join our <a href="https://t.me/@remna_shop">community</a>.</i>
